...
const { getArticalAsync } require('../action/action.js');
...
class Artical extends React.Component {
	...
	componentWillMount(){
		...
		if(shouldFetch) dispatch(getArticalAsync(id));//不存在向服务器请求数据 更新state 然后render
	}
...
module.exports = connect(aaa)(Artical)
